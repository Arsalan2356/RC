				...
				while (bitboard) {
					...
					if (!(target_square < a8) &&
						...
						if (source_square >= a7 && source_square <= h7) {
							add_move(move_list,
								Move::create_id(source_square, target_square,
									piece, Q, 0, 0, 0, 0, 12));
							add_move(move_list,
								Move::create_id(source_square, target_square,
									piece, R, 0, 0, 0, 0, 12));
							...
							if ((source_square >= a2 && source_square <= h2) &&
								...
								add_move(move_list,
									Move::create_id(source_square,
										target_square - 8, piece, 0, 0, 1, 0, 0,
										12));
						}
					}
					...
					attacks =
						pawn_attacks[side][source_square] & occupancies[black];
					...
					while (attacks) {
						...
						for (int bb_piece = 0; bb_piece < 6; bb_piece++) {
							int piece_pos =
								(side == white ? bb_piece + 6 : bb_piece);
						...
						if (source_square >= a7 && source_square <= h7) {
							add_move(move_list,
								Move::create_id(source_square, target_square,
									piece, Q, 1, 0, 0, 0, target_piece));
							add_move(move_list,
								Move::create_id(source_square, target_square,
									piece, R, 1, 0, 0, 0, target_piece));
							add_move(move_list,
								Move::create_id(source_square, target_square,
									piece, B, 1, 0, 0, 0, target_piece));
							add_move(move_list,
								Move::create_id(source_square, target_square,
									piece, N, 1, 0, 0, 0, target_piece));
						}
					...
					if (en_passant_sq != no_sq) {
						// lookup pawn attacks and bitwise AND with enpassant
						// square (bit)
						...
						if (enpassant_attacks) {
							...
							int target_enpassant =
								get_ls1b_index(enpassant_attacks);
							add_move(move_list,
								Move::create_id(source_square, target_enpassant,
									piece, 0, 1, 0, 1, 0,
									(side == white ? 6 : 0)));
						}
					}
			...
			if (piece == K) {
				...
				if (castle_rights & wk) {
					...
					if (!get_bit(occupancies[both], f1) &&
						...
						// make sure king and the f1 squares are not under
						// attacks
						if (!is_square_attacked(e1, black) &&
							!is_square_attacked(f1, black))
							add_move(move_list,
								Move::create_id(
									e1, g1, piece, 0, 0, 0, 0, 1, 12));
					}
				}
				...
				if (castle_rights & wq) {
					...
					{
						// make sure king and the d1 squares are not under
						// attacks
						if (!is_square_attacked(e1, black) &&
							!is_square_attacked(d1, black))
							add_move(move_list,
								Move::create_id(
									e1, c1, piece, 0, 0, 0, 0, 1, 12));
					}
				}
			}
		}
		...
		// generate black pawns & black king castling moves
			...
			if (piece == p) {
				...
				while (bitboard) {
					...
					// init pawn attacks bitboard
					attacks =
						pawn_attacks[side][source_square] & occupancies[white];
			...
			if (piece == k) {
				...
				if (castle_rights & bk) {
					...
					if (!get_bit(occupancies[both], f8) &&
						...
						// make sure king and the f8 squares are not under
						// attacks
						if (!is_square_attacked(e8, white) &&
							!is_square_attacked(f8, white))
							add_move(move_list,
								Move::create_id(
									e8, g8, piece, 0, 0, 0, 0, 1, 12));
					}
				}
				...
				if (castle_rights & bq) {
					...
					{
						// make sure king and the d8 squares are not under
						// attacks
						if (!is_square_attacked(e8, white) &&
							!is_square_attacked(d8, white))
							add_move(move_list,
								Move::create_id(
									e8, c8, piece, 0, 0, 0, 0, 1, 12));
					}
				}
			}
		}
		...
		if ((side == white) ? piece == N : piece == n) {
			...
			while (bitboard) {
				...
				while (attacks) {
					...
					if (target_piece == 12)
						add_move(move_list,
							Move::create_id(source_square, target_square, piece,
								0, 0, 0, 0, 0, target_piece));

					else
						// capture move
						add_move(move_list,
							Move::create_id(source_square, target_square, piece,
								0, 1, 0, 0, 0, target_piece));
		...
		if ((side == white) ? piece == N : piece == n) {
			...
			while (bitboard) {
				...
				while (attacks) {
					...
					// quiet move
					if (get_bit(((side == white) ? occupancies[black]
												 : occupancies[white]),
							target_square))
						// capture move
						add_move(move_list,
