...
void Board::init_magic_numbers() {
	for (int square = 0; square < 64; square++) {
		bishop_magic_numbers[square] =
			find_magic_number(square, bishop_relevant_bits[square], 2);

		rook_magic_numbers[square] =
			find_magic_number(square, rook_relevant_bits[square], 3);
	}
}
...
uint64_t Board::find_magic_number(int square, int relevant_bits, int flag) {
	...
	uint64_t attack_mask =
		(flag == 2 ? mask_bishop_attacks(square) : mask_rook_attacks(square));

	int occupancy_indices = (1 << relevant_bits);

	attack_function = (flag == 2 ? &Board::bishop_attacks_on_the_fly
								 : &Board::rook_attacks_on_the_fly);

	for (int index = 0; index < occupancy_indices; index++) {
		occupancies_magic[index] =
			set_occupancy(index, relevant_bits, attack_mask);

		attacks[index] =
			(this->*(this->attack_function))(square, occupancies_magic[index]);
		// attacks[index] = (flag == 2 ? bishop_attacks(square,
		// occupancies[index]) : rook_attacks(square, occupancies[index]));
	}
	...
	for (int random_count = 0; random_count < 100000000; random_count++) {
		...
		if (__builtin_popcountll(
				(magic_number * attack_mask) & 0xFF00000000000000) < 6)
			continue;
		...
		for (index = 0, fail = 0; !fail && index < occupancy_indices; index++) {
			int magic_index =
				(int) ((occupancies_magic[index] * magic_number) >>
					(64 - relevant_bits));
