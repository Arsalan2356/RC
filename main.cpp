...
void draw_piece(SDL_Renderer *renderer, int square_size, uint64_t board, int piece_pos, int side);
void highlight_last_move(SDL_Renderer *renderer, uint64_t move, int square_size, int side);
void highlight_attacked_squares(SDL_Renderer *renderer, int pos, moves *player_moves, int side);
...
int main(int argc, char *argv[])
{
	...
	while (running)
	{
		...
		if (!human_turn && !board->is_checkmate && !board->is_stalemate)
		{
			...
			uint64_t move = (use_nnue ? board->search_position_nnue(8) : board->search_position(8));
		...
		{
			highlight_last_move(renderer, board->move_log[board->move_index], SQUARE_SIZE, side);
		}
		...
		if (square_selected[0] != -1)
		{
			...
			if (piece != -1)
				highlight_attacked_squares(renderer, square_selected[0], board->player_moves, side);
		}
