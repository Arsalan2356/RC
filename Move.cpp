...
// Only constructor which takes in a square_from, a square_to, a flag variable
// with multiple values (see Move.h), a piece_moved (need to account for
// white/black), a piece_captured (-1 if none and needs to account for
// white/black), a promotion piece
// (-1 if none, not required to account for black (see to_pgn) but it's
// preferred for piece conversion)
...
Move::Move(int square_from, int square_to, int piece, int promoted,
	...
	this->move_id = (square_from) | (square_to << 6) | (piece << 12) |
		(promoted << 16) | (capture_flag << 20) | (double_flag << 21) |
		(en_passant_flag << 22) | (castle_flag << 23);
}
...
void Move::to_pgn(std::string &pgn, int diff) {
	...
	if (!(castle_flag)) {
		...
		if (capture_flag) {
			if (piece % 6 == 0) pgn += (char) (square_from % 8 + 97);
			pgn += "x";
		}
		pgn += ((char) ((square_to % 8) + 97));
		pgn += ((char) ((8 - (square_to / 8)) + 48));
		if (piece == 0 && (promoted != 0)) pgn += "=" + piece_names(promoted);
	}
	else {
...
uint64_t Move::create_id(int square_from, int square_to, int piece,
	int promoted, int capture_flag, int double_flag, int en_passant_flag,
	int castle_flag, int capture_piece) {
	return ((square_from) | (square_to << 6) | (piece << 12) |
		(promoted << 16) | (capture_flag << 20) | (double_flag << 21) |
		(en_passant_flag << 22) | (castle_flag << 23) | (capture_piece << 24));
}
